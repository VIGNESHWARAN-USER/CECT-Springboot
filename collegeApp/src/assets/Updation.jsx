...
const Updation = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post("http://localhost:8080/api/markupdation", {
        _id: detail._id,
  ...
  useEffect(() => {
    ...
    const fetchDetails = async () => {
      try {
        const response = await axios.post('http://localhost:8080/api/getDetailsUpdate', { email:user.email });
  ...
  const handleCourseChange = (e) => {
    const newCourseTitle = e.target.value; // Get the selected course title directly
    setSelectedCourseTitle(newCourseTitle); // Update the state
    console.log(newCourseTitle); // Log the new title (immediate value)
  
    const foundItem = details.find((item) => item.courseTitle === newCourseTitle); // Use the value directly
    if (foundItem) {
      setdetail(foundItem); // Set the ID
      console.log(foundItem); // Log the found ID
    } else {
