...
const RejectedStud = () => {
    ...
    const handleDelete = async () => {
        try {
            const response = await axios.post('http://localhost:8080/api/deleterejecteddetails',{role: user.role, 
                year: user.year,
                dept:user.dept,
                sec:user.sec });
    ...
    useEffect(() => {
        ...
        const fetchDetails = async () => {
            try {
                const endpoint = 'http://localhost:8080/api/detailsrejected';
                const response = await axios.post(endpoint,{role: user.role, 
                    year: user.year,
                    dept:user.dept,
                    sec:user.sec } );
                    if(response.data)
