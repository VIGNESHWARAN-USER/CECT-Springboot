...
function Login({ setEmail, email, setOTP, setname, setuser, setrole }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post("http://localhost:8080/api/login", { Email: email, pass });
      if (response.status === 200) {
        console.log(response.data)
        const val = `${response.data.first} ${response.data.last}`;
        setname(val);
  ...
  const navigateToOtp = async () => {
    if (email) {
      ...
      try {
        const response = await axios.post("http://localhost:8080/api/send_recovery_email", { OTP, recipient_email: email });
        console.log(response);
        if (response.status === 200) {
          setErr("");
          navigate("../otp");
        } else {
          setErr("Mail has not been sent");
        }
      } catch (error) {
        setErr("Internal server error");
      }
    ...
    <div className="flex h-screen bg-green-100">
      ...
      <div className="min-h-screen bg-gradient-to-b from-green-200 to-green-400 flex flex-col items-center justify-center p-4 w-full">
        ...
        <img src={logo} alt="CECT Portal" className="w-20 h-20 mb-4 rounded-full mx-auto" />
          ...
          <form onSubmit={handleSubmit} className="mt-6 w-3/4 space-y-4">
            <div>
