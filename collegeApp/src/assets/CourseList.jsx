...
const CourseManagementPage = () => {
  ...
  const handleSave = async () => {
    try 
    {
      const response = await axios.post("http://localhost:8080/api/savecourse", newCourse);
  ...
  const handleDeleteCourse = async (id) => {
    try 
    {
      await axios.post(`http://localhost:8080/api/deletecourse/${id}`);
  ...
  const handleDeleteAllCourses = async () => {
    try 
    {
      await axios.post("http://localhost:8080/api/deletecourses");
      setCourses([]);
            ...
            <table className="w-full border-collapse bg-green-100/70 backdrop-blur-lg rounded-lg">
              <thead>
                <tr className="bg-green-600 text-white text-left">
                  ...
                  <th className="py-4 px-3 sm:px-6 text-xs sm:text-sm md:text-base font-semibold">Generated Course Name</th>
              ...
              <tr>
                ...
                {courses.map((course) => (
                  <tr key={course._id} className="hover:bg-green-100 transition-all duration-200">
                    ...
                    <td className="py-4 px-3 sm:px-6 text-xs sm:text-sm md:text-base">{course.orgcoursecode}</td>
