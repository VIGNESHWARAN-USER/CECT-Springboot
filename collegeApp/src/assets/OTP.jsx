...
function OTP({ email, otp }) {
  ...
  const resendOTP = () => {
    if (disable) return;
    axios
      .post("http://localhost:8080/api/send_recovery_email", { OTP: otp, recipient_email: email })
      .then(() => {
        setDisable(true);
        setTimer(60);
          ...
          <div className="flex justify-center mb-8">
            {OTPinput.map((_, index) => (
              <input
                key={index}
                type="text"
                className="focus:outline-none text-xl px-4 py-2 mx-2 border border-gray-300 rounded-lg text-center focus:ring-2 focus:ring-green-400 focus:border-green-500"
                maxLength="1"
                required
                ref={inputRefs[index]}
