...
const Forms = ({ mail }) => {
  ...
  const storedMail = (JSON.parse(localStorage.getItem('user')).email)
  ...
  useEffect(() => {
    ...
    const fetchDetails = async () => {
      try {
        console.log("Hi")
        const response = await axios.post('http://localhost:8080/api/getDetails', { email:storedMail });
        const response1 = await axios.post('http://localhost:8080/api/getDropDetails',{ email:storedMail });
